...
export class FixButtonComponent {
  ...
  handleAction() {
    ...
    if (action === 'DELETE_INACTIVE_EVENT') {
      this.contextMenuService.handleMultiEventAction('DELETE_SELECTED_EVENTS', [
        event,
      ]);
    } else {
      this.contextMenuService.handleAction(action, event, payload);
    }
  }
}
